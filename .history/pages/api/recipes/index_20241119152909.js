...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Extract username from Authorization header if needed
    const username = authHeader ? authHeader.replace('Bearer ', '') : null;
    ...
    if (req.method === "POST") {
      console.log("Processing POST request for user:", token.username);
      ...
      // Get user
      console.log("Finding user:", token.username);
      const user = await prisma.user.findUnique({
        where: { username: username || token?.username || session?.user?.username }
      });
